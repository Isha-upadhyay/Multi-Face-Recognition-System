...
def create_face_embeddings():
    ...
    knownEncodings = [] # creating empty list for encodings of the images
    knownNames = [] # creating empty list for name of the images

    # loop over the image paths
    for (i, imagePath) in enumerate(imagePaths):
        # extract the person name from the image path
        print("[INFO] processing image {}/{}".format(i + 1, len(imagePaths))) # printing the image number
        name = imagePath.split(os.path.sep)[-2] # extracting the name of the person from the image path
        print(name) 
        ...
        encoding = face_recognition.face_encodings(image, 
                                                   ...
                                                   model='large')[0] #model='large' or 'small' can be used. 'large' is more accurate but slower than 'small' model
